...
Sandbox = function(opt){
  ...
  if (!opt.window) {
    ...
    sa = opt.window;
    if (!sa.name) {
      sa.name = "sandboxjs-" + Math.random().toString(36).substring(2);
    }
    this.window = window.open("", sa.name, "width=" + (sa.width || 480) + ",height=" + (sa.height || 640));
  }
  return this;
};
Sandbox.prototype = import$(Object.create(Object.prototype), {
  ...
  send: function(msg){
    return (this.window || this.root.contentWindow).postMessage({
      type: 'msg',
      payload: msg
    }, '*');
  },
  ...
  reload: function(){
    ...
    return new Promise(function(res, rej){
      var ref$;
      if (this$.root) {
        return ref$ = this$.root, ref$.onload = function(){
          return res(this$.url);
        }, ref$.src = this$.url, ref$;
      } else {
        return window.open(this$.url, this$.opt.window.name);
      }
    });
  },
  load: function(d){
